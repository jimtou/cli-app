...
run_electrum payto <fixd_address> <small_amount>
...
const fixd_address = '1ZeZ6augvZez9itnxZpRCySAVJytH88DB';
...
const test_iota = 'UEFZCKSFNDLELSPZZKVMLUVBWTDFTXGMNNNOQGZCTJZWCTYFYATMRYOJUEH9C9RIHSJBXMZAWDAIWPVPBXQKTDUHVD';
...
const setupBTCCommand = (data, iotajs, vorpal) => {
  vorpal
    ...
    .action((args, callback) => {
      const btc_program = "../bcc-electrum/electrum"
      if (args.address !== 'test') {
        const compact_iota = compress_tryte(args.address);
        const cmd = btc_program + " payto " + fixd_address + ' 0.312 --attachData ' + compact_iota
